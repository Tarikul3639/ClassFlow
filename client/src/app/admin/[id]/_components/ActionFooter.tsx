...
import { createNewEventThunk } from "@/redux/slices/admin/events/thunks/createNewEventThunks";
import { updateEventThunk } from "@/redux/slices/admin/events/thunks/updateEventThunks";
...
export const ActionFooter = ({ form }: ActionFooterProps) => {
  ...
  const status = useAppSelector(
    (state) => state.admin.status[form._id === "new" ? "creating" : form._id],
  );
  ...
  const handleSubmit = async () => {
    try {
      setLoading(true);
      if (form._id === "new") {
        await dispatch(createNewEventThunk(form)).unwrap();
        router.push("/admin");
      } else {
  ...
  const isBusy = loading || status?.creating || status?.updating;
